<template>
  <div class="popup-container">
    ...
    <div class="content">
      ...
      <div v-if="nativeConnectionStatus === 'disconnected' && !isConnecting && showSetupBanner" class="setup-banner">
        <div class="setup-banner-header">
          <span class="setup-icon">🚀</span>
          <h3 class="setup-title">{{ getMessage('setupRequiredTitle') || 'Quick Setup Required' }}</h3>
          <button class="close-banner-btn" @click="dismissSetupBanner">✕</button>
        </div>
        <p class="setup-description">
          {{ getMessage('setupRequiredDescription') || 'Download and run the companion app to enable Chrome MCP features.' }}
        </p>
        <div class="setup-actions">
          <a 
            :href="companionAppDownloadUrl" 
            target="_blank" 
            class="download-button"
          >
            <DownloadIcon />
            <span>{{ getMessage('downloadCompanionApp') || 'Download Companion App' }}</span>
          </a>
          <button class="setup-help-button" @click="showSetupHelp = !showSetupHelp">
            {{ getMessage('setupHelpButton') || 'Setup Help' }}
          </button>
        </div>
        <div v-if="showSetupHelp" class="setup-help-content">
          <ol class="setup-steps">
            <li>{{ getMessage('setupStep1') || 'Download the ChromeMCPBridge-Setup.exe' }}</li>
            <li>{{ getMessage('setupStep2') || 'Double-click to run the installer' }}</li>
            <li>{{ getMessage('setupStep3') || 'Click "Connect" button below' }}</li>
            <li>{{ getMessage('setupStep4') || 'Done! MCP server is now active' }}</li>
          </ol>
        </div>
      </div>
...
const companionAppDownloadUrl = ref('https://github.com/anthropics/mcp-chrome/releases/latest/download/ChromeMCPBridge-Setup.exe');

// Dismiss setup banner and save preference
const dismissSetupBanner = () => {
  showSetupBanner.value = false;
  chrome.storage.local.set({ setupBannerDismissed: true });
};

// Load setup banner preference
const loadSetupBannerPreference = async () => {
  try {
    const result = await chrome.storage.local.get(['setupBannerDismissed']);
    if (result.setupBannerDismissed) {
      showSetupBanner.value = false;
    }
  } catch (error) {
    console.error('Failed to load setup banner preference:', error);
  }
};
...
onMounted(async () => {
  ...
  await loadSetupBannerPreference();
