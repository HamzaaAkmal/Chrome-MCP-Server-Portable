...
class InjectScriptTool extends BaseBrowserToolExecutor {
  ...
  async execute(args: InjectScriptParam & ScriptConfig): Promise<ToolResult> {
    try {
      ...
      // Check URL restrictions for script injection
      if (
        ...
        return createErrorResponse(
          'Cannot inject scripts into special browser pages, web store pages, or extension pages due to security restrictions. These pages include chrome://, edge://, extension pages, and browser web stores.',
        );
      }
...
class SendCommandToInjectScriptTool extends BaseBrowserToolExecutor {
  ...
  async execute(args: SendCommandToInjectScriptToolParam): Promise<ToolResult> {
    try {
      ...
      // Get tab details to check URL restrictions
      ...
      if (
        ...
        return createErrorResponse(
          'Cannot send commands to scripts in special browser pages, web store pages, or extension pages due to security restrictions.',
        );
      }
...
async function handleInject(tabId: number, scriptConfig: ScriptConfig) {
  ...
  try {
    ...
    // Provide specific error message for Chrome URL access restrictions
    if (errorMessage.includes('Cannot access a chrome:') || errorMessage.includes('chrome://')) {
      throw new Error('Cannot access a chrome:// URL - script injection is not allowed on browser internal pages');
    }
    
    throw new Error(`Script injection failed: ${errorMessage}`);
  }
}
